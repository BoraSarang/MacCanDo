// [FEATURE] T-89: SEO 패널 — 메타 프리뷰/편집/자동생성 (v2.15)
import SwiftUI

struct SEOPanel: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                ...
                Button("수동 편집") { showEditor.toggle() }
    ...
    private func generateSEO() async {
        ...
        do {
            ...
            seoMeta = meta
            DebugLogger.info("SEOPanel", "[FEATURE] SEO 메타 자동 생성 완료")
        } catch {
            errorMessage = (error as? APIError)?.message ?? error.localizedDescription
            DebugLogger.error("SEOPanel", "SEO 생성 실패: \(error)")
        }
    }
