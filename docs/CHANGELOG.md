...
- T-95: JSON-LD 구조화 데이터 구현 (`web/app/post/[slug]/page.tsx`)
  - schema.org `BlogPosting`: headline/description/image/datePublished/dateModified/author/publisher/mainEntityOfPage/keywords/articleSection
  - `NEXT_PUBLIC_SITE_URL` 환경변수 기준 절대 URL (기본 http://localhost:3000)
  - E2E skip 해제 → BlogPosting 타입+필수 필드 검증으로 강화
- 총 7 passed / 0 skipped (workspace-flow.spec.ts)

### 검증

- macOS: `xcodebuild -configuration Debug BUILD SUCCEEDED` — 앱 실행 확인 (`/Users/lee/Applications/MacCanDo.app` pid 95086)
- Web: `next build` 통과, Playwright E2E 7/7 passed (workspace-flow) + smoke 4
- E2E 실행 시간: ~19초
